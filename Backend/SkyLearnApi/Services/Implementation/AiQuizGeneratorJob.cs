            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(jobParams.TempPdfPath))
                {
                    ...
                    if (File.Exists(fullPath))
                    {
                        ...
                        sourceContent.AppendLine("--- See Attached PDF File ---");
                    }
                }

                var questionTypes = jobParams.QuestionTypes ?? "MCQ";
                var prompt = $@"Based on the following educational content, generate exactly {jobParams.NumberOfQuestions} questions.

...
class _DepartmentsScreenState extends State<DepartmentsScreen> {
  ...
  DataRow _buildRow(BuildContext context, GetAllDepartmentModel dep) {
    return DataRow(
      cells: [
        DataCell(Text(dep.name)),
        DataCell(
          SizedBox(
            width: 200,
            child: Text(
        ...
        DataCell(Text(dep.headName)),
        DataCell(
          Text(
            dep.years.isEmpty
                ? "-"
                : dep.years.map((e) => e.name).join(", "),
          ),
        ),
        DataCell(Text(dep.id.toString())),

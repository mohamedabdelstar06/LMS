...
class _UpdateDepartmentPageState extends State<UpdateDepartmentPage> {
            ...
            Expanded(
              child: BlocConsumer<DepartmentsCubit, DepartmentsState>(
                ...
                builder: (context, state) {
                  if (state is DepartmentByIdLoading || state is UpdateDepartmentLoading) {
                  ...
                  if (state is DepartmentsError || state is DepartmentByIdError) {
                    final message = state is DepartmentsError ? state.message : (state as DepartmentByIdError).message;
                    return Center(
                      child: Column(
  ...
  Widget _buildField(String label, TextEditingController controller, {int maxLines = 1}) {
    return Column(
      ...
      children: [
        Text(label, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF2563EB))),
  ...
  Widget _buildUploadArea(String? currentImageUrl) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(16), // Slightly rounded container
          border: Border.all(
            color: selectedImageBytes != null ? const Color(0xFF2563EB) : const Color(0xFFE2E8F0),
            width: 2,
          ),
        ...
        child: selectedImageBytes != null
            ? Stack(
                children: [
                  // --- CHANGE 1: Updated BorderRadius for the image ---
                 Container(
                    width: double.infinity,
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: const Text('New Image', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
                    ),
                  ),
                  Positioned.fill(
                    child: Container(
                      ...
                      child: const Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 8),
                            Text('Click to change', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
                          ],
                        ),
                      ),
                    ),
              ...
              borderRadius: BorderRadius.circular(16),
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          child: const Text('Current Image', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
                        ),
                      ),
                      Positioned.fill(
                        child: Container(
                          decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: Colors.black.withOpacity(0.2)),
                          child: const Center(
                            child: Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Icon(Icons.cloud_upload_outlined, size: 48, color: Colors.white),
                                SizedBox(height: 8),
                                Text('Click to change image', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
                              ],
                            ),
                          ),
                        ),
                ...
                : Column(
                    ...
                    children: const [
                      ...
                      SizedBox(height: 12),
                      Text("Upload Department Image", style: TextStyle(color: Color(0xFF64748B), fontWeight: FontWeight.w600)),
                      SizedBox(height: 4),
                      Text("Click to upload", style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12)),
                    ],
                  ),
      ),
    );
  ...
  Widget _buildActionButtons(DepartmentByIdLoaded state) {
    final isLoading = context.watch<DepartmentsCubit>().state is UpdateDepartmentLoading;
    final department = state.department;
      ...
      children: [
        Expanded(
          child: OutlinedButton.icon(
            onPressed: isLoading ? null : () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const AdminCourseScreen())),
        ...
        Expanded(
          ...
          child: InkWell(
            ...
            child: Container(
              height: 55,
              decoration: BoxDecoration(
                gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF1849A9), Color(0xFF53B1FD)]),
                borderRadius: BorderRadius.circular(12),
              ),
              child: Center(
                child: isLoading
                    ? const Row(
                        ...
                        children: [
                          ...
                          SizedBox(width: 12),
                          Text("Updating Department...", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
                        ],
                      )
                    : const Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Text("Update Department", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
                          SizedBox(width: 8),
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 16),
                  const Text("Update Department", style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.black)),
                ],
              ),
                      ...
                      builder: (context, userState) {
                        if (userState is UsersLoadingState) {
                          return Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text("Department Head", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF2563EB))),
                              const SizedBox(height: 8),
                              Container(
                        ...
                        // 3. Error State
                          ...
                          return _buildDropdownField(
                            "Error loading users",
                            [UserLiteModel(fullName: state.department.headName, role: "Current Head (API Error)", id: state.department.id)],
                            isHeadExpanded,
                            (user) => setState(() {
                              selectedHeadName = user.fullName;
                              selectedHeadId = user.id;
                        ...
                        // 4. Initial/Fallback State
                          ...
                          return _buildDropdownField(
                            selectedHeadName,
                            [UserLiteModel(fullName: state.department.headName, role: "Current Head", id: state.department.id)],
                            isHeadExpanded,
                            (user) => setState(() {
                              selectedHeadName = user.fullName;
                              selectedHeadId = user.id;
              ...
              const SizedBox(height: 24),
              const Text("Photo", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF2563EB))),
              const SizedBox(height: 8),
      ...
      children: [
          ...
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: isExpanded ? const Color(0xFF2563EB) : const Color(0xFFE2E8F0), width: isExpanded ? 2 : 1),
              ),
              child: Row(
        ...
        if (isExpanded)
          Container(
            ...
            child: users.isEmpty
                ? const Padding(padding: EdgeInsets.all(16), child: Center(child: Text('No users available', style: TextStyle(color: Color(0xFF64748B)))))
                : ListView.builder(
                    shrinkWrap: true,
                          ...
                          child: Row(
                            children: [
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: user.role == "Admin" ? Colors.purple.withOpacity(0.1) : Colors.orange.withOpacity(0.1),
                                  borderRadius: BorderRadius.circular(6),
                                ),
                                child: Icon(
                                  user.role == "Admin" ? Icons.admin_panel_settings : Icons.school,
                                  size: 16,
                                  color: user.role == "Admin" ? Colors.purple : Colors.orange,
                                ),
                              ),
                              const SizedBox(width: 12),
                              Expanded(
